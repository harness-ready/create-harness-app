 ...
 */
// ─────────────────────────────────────────────────
// 2c. MCP (Model Context Protocol) server config (issue #9)
// ─────────────────────────────────────────────────

/**
 * generateMcpJson — project-scoped MCP server config (`.mcp.json`), read by
 * Claude Code and other MCP-aware tools. Emitted when `answers.mcp` is set and
 * the user chose Claude Code or "Multiple"; null otherwise.
 *
 * Note on Codex/Copilot: neither defines a project-scoped MCP file, so no
 * dedicated config is emitted for them (AGENTS.md already carries context).
 */
export function generateMcpJson(answers) {
  if (!answers.mcp) return null;
  if (answers.codingAgent !== 'claude-code' && answers.codingAgent !== 'multiple') return null;
  return {
    filepath: '.mcp.json',
    content: `{
  "mcpServers": {
...
/** `.cursor/mcp.json` — Cursor's MCP server config, when Cursor was chosen. */
export function generateCursorMcp(answers) {
  if (!answers.mcp) return null;
  if (answers.codingAgent !== 'cursor' && answers.codingAgent !== 'multiple') return null;
  return {
...
export function generateAll(answers) {
  const artefacts = [
    ...
    generateMcpJson(answers),
    generateCursorMcp(answers),
