...
// Optional extra "sensors" — feedback tooling beyond lint/test/CI. Selected via
// the `sensors` answer (interactive multi-select, `--sensors a,b`, or config).
export const SENSORS = [
  { value: 'dependabot', label: 'Dependabot (dependency update PRs)' },
  { value: 'gitleaks', label: 'gitleaks (secret scanning config)' },
  { value: 'commitlint', label: 'commitlint (conventional commits)' },
  { value: 'docker', label: 'Dockerfile (container build)' },
];

// Map a language to its Dependabot package ecosystem.
export const DEPENDABOT_ECOSYSTEM = {
  typescript: 'npm',
  python: 'pip',
  go: 'gomod',
  java: 'maven',
