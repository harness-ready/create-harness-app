...
export async function main() {
  ...
  try {
    if (positionals[0] === 'init') {
      await runInit(rl, positionals.slice(1), locale, config, useDefaults, sensorsDefault);
      return;
    }
...
async function runInit(rl, positional, locale, config, useDefaults, sensorsDefault) {
