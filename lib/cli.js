...
const VALUE_FLAGS = new Set(['--locale', '--config']);
...
export async function main() {
  ...
  try {
    if (positionals[0] === 'init') {
      await runInit(rl, positionals.slice(1), locale, config, useDefaults);
      return;
    }
...
async function runInit(rl, positional, locale, config, useDefaults) {
