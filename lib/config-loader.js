...
export function loadConfig({ configPath, cwd = process.cwd(), warn = (m) => console.warn(m) } = {}) {
  ...
  if (!path) return { values: {}, extraConventions: [], sensors: [], mcp: false, path: null };
  ...
  try {
    ...
    return { values: {}, extraConventions: [], sensors: [], mcp: false, path };
  }
  ...
  const sensors = Array.isArray(raw.sensors)
    ? raw.sensors.filter((s) => typeof s === 'string' && sensorKeys.has(s))
    : [];

  const mcp = raw.mcp === true;

  return { values, extraConventions, sensors, mcp, path };
}
